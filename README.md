...
The PB600 uses three custom Lua mixer scripts:
  ...
  `blade.lua`                        Front blade actuator control,
                                     positioning, manual operation, and
                                     coordinated blade movement
...
tiller raises. Because the Engine signal is derived from the actual
track outputs, the engine remains near idle until reverse movement is
permitted.
...
while the tiller is completing its automatic reverse-clearance lift.
...
When operating in Groom mode, reverse is integrated with the rear tiller
rather than simply being blocked.
...
prohibited in Groom unless the operator held the `SH` switch.
...
live from the transmitter.
  ...
  GV4         Reverse Lift   0-100       10          Amount the tiller
              Height                                 raises before reverse
                                                     is permitted
...
``` text
Reverse lift UP: 17.0 x 0.10 = 1.70 seconds
